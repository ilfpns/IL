...
- TPG
    ...
    - TPG : 정해진 패턴을 스스로 만들어내는 회로이다. 카메라 센서가 없을 때 사용한다
    - VDMA : AXI4-Stream은 주소가 없어 CPU를 거치지 않은 DMA방식으로 레지스터에 rw을 진행해야 함. stream to mem (S2MM) 방식으로 stream 전송
    - axil_regfile : AXI4-Lite를 슬레이브로 노출 (제어 가능하도록)
        - stream IP는 대부분 AXI-Lite port가 없다. 그렇기 때문에 TPG 파라미터 제어를 위해 axil_regfile을 만들었다.
    
    AXI 즉 고속 주변장치 버스는 하나의 종류가 아니다
    
    - AXI4-Stream : 주소 없이 데이터가 그냥 흘러간다 (TPG → 처리 체인 → VDMA)
    - AXI4-Lite : 주소 있이 한 번에 32비트씩, 레지스터를 설정한다. (axil_regfile)
    - AXI4 : 주소 있이 버스트 전송으로 빠르다 (VDMA → DDR)
    
    !image.png
    
    TPG IP에서 데이터를 운반하는 방식에 VDMA가 사용되었고, TPG IP안에 제어 레지스터가 없어서 이것을 제어하기 위해 만든 레지스터 묶음 (register file)파일이 axil_regfile이다
    
    실제 카메라가 있다면 vdma-capture와 axil-regfile은 바뀌지 않는다. 
    
    그저 센서 I2C 드라이버를 다시 작성한다.
    
    실물 카메라를 장착하고 id가 읽힌다면 PL bitstream이 제대로 올라갔고, 이제 CTRL을 조작하여 실물 값을 받으면 된다.
